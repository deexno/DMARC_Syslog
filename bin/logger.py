...
class SyslogClient:
    ...
    _application_name = "DMARC2Syslog"
    _logger_name = "syslog"
    _datefmt = "%Y-%m-%dT%H:%M:%S%z"
    _log_format = (
        "%(asctime)s " + _hostname + " " + _application_name + " %(message)s"
    )

    @staticmethod
    def initiate(syslog_server, syslog_port):
        # Initialize the SyslogClient with the provided syslog server and port
        handler = logging.handlers.SysLogHandler(
            address=(syslog_server, syslog_port)
        )
        handler.formatter = logging.Formatter(
            SyslogClient._log_format, datefmt=SyslogClient._datefmt
        )

        SyslogClient._syslog_logger = logging.getLogger(
            name=SyslogClient._logger_name
        )
    ...
    def log_info_msg(msg):
        # Log an info message using the syslog logger
...
class ExceptlogClient:
    ...
    def initiate(error_log_path, error_log_enable, debug_log_enable):
        # Initialize the ExceptlogClient with the provided error log path,
        # error log enable flag, and debug log enable flag
        ...
        handler.formatter = logging.Formatter(
            ExceptlogClient._Log_Format, datefmt=ExceptlogClient._datefmt
        )

        ExceptlogClient._file_logger = logging.getLogger(
            name=ExceptlogClient._logger_name
        )
    ...
    def log_except(exception):
        # Log an exception using the file logger
